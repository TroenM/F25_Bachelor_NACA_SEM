...
def mesh_gen_uniform_2D_grid(N_rows: int, N_cols: int,gridtype: str) -> meshio.Mesh:
    ...
    if gridtype.lower() == "quad":
        ...
        cells = [(gridtype, triangles.astype(int).tolist())]
        return meshio.Mesh(points=points, cells=cells)
    else:
...
def plot_mesh(mesh: meshio.Mesh) -> None:
    ...
    cell_coords = [[points[int(point)] for point in cell] for cell in cells]
